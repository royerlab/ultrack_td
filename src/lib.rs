...
#[pyfunction]
fn hierarchical_segmentation(edges: Vec<(usize, usize, f32)>, num_nodes: usize, min_frontier: f32, min_size: usize, max_size: usize) -> PyResult<Vec<usize>> {
...
fn segment_mst(
    ...
    let mut edges: Vec<(usize, usize, f32, f32)> = mst
                .edge_references()
                .map(|e| (e.source().index(), e.target().index(), *e.weight(), rng.random_range(0.0..1.0)))
                .collect();
...
#[pymodule]
fn _rustlib(_py: Python, m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(hello_rust, m)?)?;
    m.add_function(wrap_pyfunction!(hierarchical_segmentation, m)?)?;
    Ok(())
}
